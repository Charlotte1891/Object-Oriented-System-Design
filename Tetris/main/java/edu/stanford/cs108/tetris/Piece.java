...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		width = xMax + 1;
		height = yMax + 1;
		// skirt stores the lowest y value
		skirt = new int[width];
		for(int i = 0; i < skirt.length; i++) {
			skirt[i] = height;
		}
		for(TPoint tp : body) {
			// change array value only when tp.y < previous value
			if(tp.y < skirt[tp.x]) {skirt[tp.x] = tp.y;} 
		}		
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		// for the new TPoint
		// the x coordinate = new height - previous TPoint.y - 1
		// the y coordinate = previous TPoint.x
		TPoint[] tpNext = new TPoint[body.length];
	    for (int i = 0;i< body.length;i++) {
	    	tpNext[i] = new TPoint(height - 1- body[i].y, body[i].x);
	    }
	    return new Piece(tpNext);
	...
	*/
	public boolean equals(Object obj) {
		...
		Set<TPoint> newPiece = new HashSet<TPoint>();
		for(TPoint tp : this.body) {
			newPiece.add(tp);
		}
		for(TPoint tp2 : other.body) {
			if(!newPiece.contains(tp2)) {
				return false;
			}
		}
		return true;
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		root.next = recursiveHelper(root, root);
		return root;
	}
	
	private static Piece recursiveHelper(Piece root, Piece curr) {
		Piece after = curr.computeNextRotation();
		if(after.equals(root)) {
			return root; 
		}else {
			after.next = recursiveHelper(root, after);
			return after;
		}
	}
